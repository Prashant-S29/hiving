...
export const aiModel = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      group: "velocity",
      description:
        "e.g. \"deepseek/deepseek-v4-flash-0731\" — the exact id from openrouter.ai/models this model corresponds to. The weekly Velocity Index automation matches on this field; leave blank to exclude this model from automated scoring.",
    }),
    defineField({
      name: "raceScore",
      title: "Velocity Index score",
      type: "number",
      group: "velocity",
      readOnly: true,
      description: "Hivig Velocity Index, 0–100. Written automatically every Sunday by the fetch-race-metrics pipeline — see RANKING_METHODOLOGY.md. Not hand-edited.",
    }),
    defineField({
      name: "previousRaceScore",
      title: "Previous Velocity Index score",
      type: "number",
      group: "velocity",
      readOnly: true,
      description: "The score from the prior automated run, kept for rank-delta display.",
    }),
    defineField({
      name: "tokensProxy",
      title: "Token volume (7d)",
      type: "number",
      group: "velocity",
      readOnly: true,
      description: "Trailing 7-day token volume from OpenRouter's rankings-daily API.",
    }),
    defineField({
      name: "downloads",
