...
export interface ArticleCardCopy {
  categoryLabels: Record<TagType, string>;
...
export function ArticleTag({ article, copy }: { article: Article; copy: ArticleCardCopy }) {
  return (
    <div className={`inline-flex items-center gap-2 font-mono text-[10px] tracking-[0.18em] uppercase border px-3 py-1 mb-4 ${TAG_COLORS[article.tagType]}`}>
      ...
      {copy.categoryLabels[article.tagType]}
    </div>
  );
}

export function FeaturedArticleCard({ article, copy }: { article: Article; copy: ArticleCardCopy }) {
  return (
    <Link
    ...
    >
      ...
      <div className="flex items-center gap-5 font-mono text-[10px] tracking-[0.1em] uppercase text-muted">
        <span>{article.readTimeMinutes} {copy.minuteReadLabel}</span>
...
export function ArticleCard({ article, copy }: { article: Article; copy: ArticleCardCopy }) {
  return (
    <Link
      href={`/intel/${article.slug.current}`}
    ...
    >
      ...
      <div className="flex items-center gap-4 font-mono text-[10px] tracking-[0.1em] uppercase text-muted">
        <span>{article.readTimeMinutes} {copy.minuteShortLabel}</span>
        <span className="text-dim">·</span>
        <span>{formatDate(article.publishedAt)}</span>
      </div>
    </Link>
